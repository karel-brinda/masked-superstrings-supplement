...
def maxNumZeros(maskedSuperstring, k):
    ...
    for i, x in enumerate(maskedSuperstring):
        ...
        if Qenc in K:
            ...
            K.remove(
                Qenc
            )  #the kmer is no longer needed and shouldn't reappear unmasked
        else:
...
def main():
    ...
    newMaskedSuperstring = maxNumZeros(maskedSuperstring=s, k=args.k)
